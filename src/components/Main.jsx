...
<div className="weather__main-temperature">
    {dailyData ?
<h2>{dailyData.current.temp_c.toFixed(0)}<span>&deg;C</span> </h2> : <h2>{hourData[0].temp_c.toFixed(0)} <span>&deg;C</span></h2> }
{dailyData ?
<img src={dailyData.current.condition.icon}  alt="Weather condition icon" className='weather__main-image'/> :<img src={hourData[0].condition.icon} alt="Weather condition icon" className='weather__main-image' /> }
</div>
